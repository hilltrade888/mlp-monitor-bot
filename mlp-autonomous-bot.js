...
async function triggerRailwayDeploy() {
  if (!RAILWAY_TOKEN || !RAILWAY_PROJECT_ID || !RAILWAY_SERVICE_ID) {
    return { success: false, error: 'Railway not configured' };
  }
  ...
  try {
    ...
    return { 
      success: true, 
      deploymentId: response.data.data.deploymentCreate.id,
      status: response.data.data.deploymentCreate.status
    };
  } catch (error) {
...
async function getRailwayDeploymentStatus() {
  ...
  try {
    ...
    const deployments = response.data.data.deployments.edges.map(e => e.node);
...
async function restartRailwayService() {
  ...
  try {
    const query = `
      mutation ServiceInstanceRestart {
        serviceInstanceRestart(serviceId: "${RAILWAY_SERVICE_ID}")
      }
    `;
...
*System Status:*
✅ Multi-AI Orchestrator (Claude, Gemini, Kimi${GPT4_API_KEY ? ', GPT-4' : ''})
...
bot.onText(/\/deploy/, async (msg) => {
  ...
  bot.sendMessage(chatId, '🚀 Triggering Railway deployment...');
  ...
  if (result.success) {
    bot.sendMessage(chatId, 
      ...
      `Check status with /deployments`,
      { parse_mode: 'Markdown' }
    );
  } else {
    bot.sendMessage(chatId, `❌ Deployment failed: ${result.error}`);
  }
});

bot.onText(/\/deployments/, async (msg) => {
  const chatId = msg.chat.id;
  
  bot.sendMessage(chatId, '📊 Fetching deployment history...');
  
  const result = await getRailwayDeploymentStatus();
  ...
  if (result.success) {
    ...
    result.deployments.forEach((d, i) => {
      ...
      message += `${i + 1}. ${status} ${d.status}\n   ${time}\n\n`;
    });
    bot.sendMessage(chatId, message, { parse_mode: 'Markdown' });
  } else {
    bot.sendMessage(chatId, `❌ Failed to get deployments: ${result.error}`);
  }
});
...
bot.onText(/\/restart/, async (msg) => {
  ...
  bot.sendMessage(chatId, '🔄 Restarting Railway service...');
  
  const result = await restartRailwayService();
  
  if (result.success) {
    bot.sendMessage(chatId, '✅ Service restarted successfully!');
  } else {
    bot.sendMessage(chatId, `❌ Restart failed: ${result.error}`);
  }
});
